...
from oceanmaster.constants import ActionType, Direction
...
def move(direction: Direction):
    ...
    """
    return Action(ActionType.MOVE, {"direction": direction.value})
...
def move_speed(direction: Direction, step: int):
    ...
    """
    return Action(ActionType.MOVE, {"direction": direction.value, "step": step})
...
def harvest(direction: Direction):
    ...
    """
    return Action(ActionType.HARVEST, {"direction": direction.value})
...
def self_destruct():
    ...
    """
    return Action(ActionType.SELF_DESTRUCT, {"direction": "NULL"})
...
def defend():
    ...
    """
    return Action(ActionType.DEFEND, {"direction": "NULL"})


def spawn(abilities: list[str], location: int):
    """
    Creates a spawn action with specified abilities at a given location.
    """
    bot_id = BOT_ID_ALLOCATOR.allocate()
    return bot_id, {"abilities": abilities, "location": {"x": location, "y": 0}}
...
def lockpick(location: Point):
    ...
    """
    return Action(ActionType.LOCKPICK, {"location": location})
